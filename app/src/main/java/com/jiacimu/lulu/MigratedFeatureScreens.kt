...
private val FeatureInk = Color(0xFF343434)
...
fun MemoryFeatureScreen(onBack: () -> Unit) {
    ...
    Scaffold(containerColor = FeaturePaper, topBar = { FeatureTopBar("记忆", onBack) }) { padding ->
        LazyColumn(
            ...
            if (memories.isEmpty()) {
                item { FeatureEmpty("还没有形成记忆", "之后接入旧项目的记忆服务和数据迁移后，会在这里显示事实、情绪和时间线。") }
            } else {
                items(memories, key = { it.id }) { memory ->
                    FeatureCardBox {
                        Text(memory.kind.name, color = FeatureBlueGray, fontSize = 12.sp)
                        Text(memory.content, color = FeatureInk, fontSize = 16.sp)
                    }
                }
            }
        }
...
fun LexiconFeatureScreen(onBack: () -> Unit) {
    ...
    val section = sections[selected].first
    val entries by LuluRepositories.lexicon.observeEntries("lulu", section).collectAsState(initial = emptyList())
    ...
    Scaffold(containerColor = FeaturePaper, topBar = { FeatureTopBar("辞海", onBack) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding)) {
            FeatureCardBox(Modifier.padding(16.dp)) {
...
fun PerformanceFeatureScreen(onBack: () -> Unit) {
    ...
    val usage by LuluRepositories.performance.observeTokenUsage().collectAsState(initial = com.jiacimu.lulu.core.TokenUsage(0, 0))
    val durations by LuluRepositories.performance.observeDurations().collectAsState(initial = com.jiacimu.lulu.core.DurationSummary(0, 0, 0))
    var selected by remember { mutableIntStateOf(0) }
        ...
        Column(Modifier.fillMaxSize().padding(padding)) {
            ...
            when (selected) {
                0 -> LazyColumn(contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    else items(errors) { FeatureCardBox { Text(it) } }
                }
                1 -> FeatureEmpty("缓存", "缓存清理能力已经接入数据层，下一步连接真实图片、模型与临时文件缓存。")
...
@Composable
private fun FeatureCardBox(modifier: Modifier = Modifier, content: @Composable Column.() -> Unit) {
    Card(
        modifier = modifier.fillMaxWidth(),
