...
/**
 * Keeps the visible inventory ledger and aggregate resource counters describing the same reality.
 *
 * The writer receipt remains the primary source. This layer repairs common model mistakes without a
 * second model call: bulk packages are expanded into consumable units, completed purchases that were
 * narrated but omitted from discoverAssets are recovered conservatively, and clearly narrated
 * consumption of an existing named item can fill a missing negative inventoryChanges entry.
 */
internal fun reconcileApocalypseInventoryOutcomeV5(
    ...
    val normalizedReported = outcome.delta.discoverAssets.map { asset ->
        normalizeApocalypseBulkAssetV5(asset, visibleResult)
    }
    ...
    val reportedChanges = outcome.inventoryChanges
        .map { change ->
            change.copy(
                ...
                tag = ensureApocalypseUnitTagV5(change.tag, inferApocalypseInventoryUnitV5(change.kind, change.title)),
            )
        }
        // Some models return the same newly acquired item in both arrays. discoverAssets owns
        // acquisition; inventoryChanges owns later changes to existing stock.
        .filterNot { change ->
            change.quantityDelta > 0 && additions.any { addition ->
                sameApocalypseInventoryIdentityV5(addition.kind, addition.title, change.kind, change.title)
            }
        }
    ...
    val changes = (reportedChanges + recoveredConsumption)
        .distinctBy { change ->
            val identity = normalizeApocalypseInventoryTitleKeyV5(change.title)
            "${change.id}|${change.kind}|$identity|${change.quantityDelta}"
        }
        .take(40)

    val concreteDelta = mutableMapOf<ApocalypseV3AssetKind, Int>()
    additions.forEach { asset -> concreteDelta[asset.kind] = (concreteDelta[asset.kind] ?: 0) + asset.quantity }
    changes.forEach { change -> concreteDelta[change.kind] = (concreteDelta[change.kind] ?: 0) + change.quantityDelta }
    ...
    fun ownConcreteDelta(kind: ApocalypseV3AssetKind, deltaField: String, afterField: String): Int? {
        val value = concreteDelta[kind] ?: return null
        // Once concrete items exist, they are the source of truth. Do not let a model's case-count
        // or rough aggregate after-value override 10 cases × 12 bottles = 120 bottles.
    ...
    return outcome.copy(
        ...
        delta = outcome.delta.copy(
            ...
            // The matching *After fields were removed above when concrete stock owns the result.
...
/**
 * The older merge helper intentionally retained only 90 assets. Preserve the larger ledger here so
 * long-running saves and large shopping trips do not silently forget older distinct items.
 */
internal fun preserveApocalypseInventoryLedgerV5(
    ...
    val current = beat.nextDirector.assets.toMutableList()
    val changedIdentities = outcome.inventoryChanges.map { change ->
        change.id to normalizeApocalypseInventoryTitleKeyV5(change.title)
    }
    val restoredOlder = save.director.assets.filter { old ->
        val stillPresent = current.any { now ->
            now.id == old.id || sameApocalypseInventoryIdentityV5(now.kind, now.title, old.kind, old.title)
        }
        if (stillPresent) return@filter false
        // A negative change may have intentionally reduced an item to zero. Never resurrect it merely
        // because the lower merge layer omitted zero-quantity assets.
        val intentionallyChanged = outcome.inventoryChanges.any { change ->
            change.quantityDelta < 0 && (
                (change.id.isNotBlank() && change.id == old.id) ||
                    sameApocalypseInventoryIdentityV5(change.kind, change.title, old.kind, old.title)
                )
        }
        !intentionallyChanged
    }
    val combined = (restoredOlder + current)
        .distinctBy { asset -> "${asset.id}|${asset.kind}|${normalizeApocalypseInventoryTitleKeyV5(asset.title)}" }
        .takeLast(240)
    return beat.copy(nextDirector = beat.nextDirector.copy(assets = combined))
}
...
private val apocalypseAcquisitionWordsV5 = listOf(
    "买", "购买", "买下", "购入", "采购", "结账", "付款", "拿到", "拿下", "收下", "带走", "搬走", "搬进", "装进", "塞进", "收入空间", "收进空间", "囤下",
)
private val apocalypseAcquisitionNegationsV5 = listOf(
    "没买", "没有买", "未买", "买不到", "没拿到", "未拿到", "缺货", "暂不买", "不买", "想买", "打算买", "准备买", "问价", "询价",
)
private val apocalypseConsumptionWordsV5 = listOf(
    "吃了", "吃掉", "吃下", "喝了", "喝掉", "饮用", "服用", "用了", "用掉", "消耗", "拆开吃", "开了一罐", "开了一瓶",
)
private val apocalypseConsumptionNegationsV5 = listOf(
    "没吃", "没有吃", "不吃", "没喝", "没有喝", "不喝", "没用", "没有用", "不用", "舍不得吃", "暂时不吃",
)
...
private fun recoverApocalypseCompletedAcquisitionsV5(text: String): List<ApocalypseV3Asset> {
    ...
    return buildList {
        apocalypseQuantityItemRegexV5.findAll(text).forEach { match ->
            ...
            val packageInfo = resolveApocalypsePackageV5(count, outerUnit, title, kind, window)
            add(
                ApocalypseV3Asset(
...
private fun normalizeApocalypseBulkAssetV5(asset: ApocalypseV3Asset, context: String): ApocalypseV3Asset {
    val source = "${asset.title} ${asset.detail} ${asset.tag} $context"
    val explicit = extractApocalypseExplicitPackageV5(source, asset.kind, asset.title)
...
private fun extractApocalypseExplicitPackageV5(
    source: String,
    ...
    val perBox = Regex("""每箱\s*(\d{1,5})\s*(瓶|罐|包|盒|袋|支|个|件|卷|桶|份|片|粒)""").find(source)
...
private fun resolveApocalypsePackageV5(
    ...
    val explicit = extractApocalypseExplicitPackageV5("$count$outerUnit$title $window", kind, title)
...
private fun defaultApocalypsePackageSizeV5(kind: ApocalypseV3AssetKind, title: String): Pair<Int, String>? {
    return when (kind) {
...
private fun recoverApocalypseNamedConsumptionV5(
    ...
    return buildList {
        existing
            .filter { it.quantity > 0 && it.kind in setOf(ApocalypseV3AssetKind.Food, ApocalypseV3AssetKind.Water, ApocalypseV3AssetKind.Medicine) }
            .forEach { asset ->
                if (alreadyReported.any { change ->
                        change.quantityDelta < 0 && (
                            (change.id.isNotBlank() && change.id == asset.id) ||
                                sameApocalypseInventoryIdentityV5(change.kind, change.title, asset.kind, asset.title)
                            )
                    }
                ) return@forEach
                ...
                names.forEach { name ->
                    ...
                    while (from < visibleText.length) {
                        ...
                        if (index < 0) break
                        val start = (index - 26).coerceAtLeast(0)
                        val end = (index + name.length + 32).coerceAtMost(visibleText.length)
                        ...
                        if (hasConsume && !negated) {
                            consumed += inferApocalypseConsumptionCountV5(visibleText, index, name.length)
                        }
...
private fun inferApocalypseConsumptionCountV5(text: String, titleIndex: Int, titleLength: Int): Int {
    val before = text.substring((titleIndex - 18).coerceAtLeast(0), titleIndex)
    val after = text.substring((titleIndex + titleLength).coerceAtMost(text.length), (titleIndex + titleLength + 18).coerceAtMost(text.length))
    val numberPattern = Regex("""(\d{1,4}|[一二两三四五六七八九十]{1,3})\s*(?:瓶|罐|包|盒|袋|份|支|片|粒|个)?\s*$""")
    numberPattern.find(before)?.groupValues?.getOrNull(1)?.let(::parseApocalypseSmallNumberV5)?.let { return it.coerceAtLeast(1) }
    val afterPattern = Regex("""^[^。；，,！？?]{0,8}?(\d{1,4}|[一二两三四五六七八九十]{1,3})\s*(?:瓶|罐|包|盒|袋|份|支|片|粒|个)""")
    afterPattern.find(after)?.groupValues?.getOrNull(1)?.let(::parseApocalypseSmallNumberV5)?.let { return it.coerceAtLeast(1) }
    return 1
}
...
private fun mergeApocalypseSceneAdditionsV5(
    ...
    recovered.forEach { item ->
        val index = merged.indexOfFirst { existing ->
            sameApocalypseInventoryIdentityV5(existing.kind, existing.title, item.kind, item.title)
        }
        if (index < 0) {
            merged += item
        } else {
            ...
            merged[index] = existing.copy(
                ...
                detail = when {
                    item.detail.isBlank() -> existing.detail
                    existing.detail.isBlank() -> item.detail
                    existing.detail.contains(item.detail) -> existing.detail
                    else -> "${existing.detail}；${item.detail}"
                }.take(360),
...
private fun cleanApocalypseInventoryTitleV5(raw: String): String {
    ...
    val stops = listOf("以及", "并且", "然后", "随后", "接着", "放进", "装进", "塞进", "收入", "收进", "带走", "搬走", "结账", "付款", "一共", "共计", "总共", "和")
    stops.map { stop -> value.indexOf(stop).takeIf { it > 0 } }.filterNotNull().minOrNull()?.let { value = value.substring(0, it) }
...
private fun inferApocalypseInventoryUnitV5(kind: ApocalypseV3AssetKind, title: String): String = when (kind) {
...
private fun ensureApocalypseUnitTagV5(tag: String, unit: String): String {
    ...
    return listOf(tag.trim('；', ';', ' '), "单位=$unit").filter(String::isNotBlank).joinToString("；")
}
