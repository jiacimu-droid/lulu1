...
object LuluDeviceToolBridge {
    ...
    suspend fun respond(
        ...
        val planner = LuluAiServices.gateway.generate(
            ...
            instruction = """
                ...
                2. get_location，args={}：读取最近真实经纬度、精度和时间。
        ...
        val plan = parsePlan(plannedReply.text)
            ?: return Result.success(plannedReply)
        if (plan.action == "reply") {
            return Result.success(plannedReply.copy(text = plan.text.ifBlank { plannedReply.text }))
        }
        if (plan.action != "tool" || plan.tool.isBlank()) {
            return Result.success(plannedReply)
        }
        ...
        when (tool.trim().lowercase()) {
            ...
            "list_alarms" -> {
                ...
                JSONObject()
                    ...
                    .put(
                        "alarms",
                        alarms.joinToString("\n") { alarm ->
                            "${alarm.id} | ${alarm.triggerAt.atZone(ZoneId.systemDefault())} | ${alarm.label}"
                        },
                    )
                    .toString()
            }
            ...
            "read_screen" -> {
                ...
                JSONObject()
                    ...
                    .put("windowTitle", value.windowTitle)
                    .put("visibleText", value.visibleText.take(6_000))
                    .put("capturedAt", value.capturedAt?.toString().orEmpty())
                    .toString()
            }
    ...
    private fun currentApp(context: Context): String {
        ...
        if (accessibility.connected && accessibility.packageName.isNotBlank()) {
            return JSONObject()
                .put("success", true)
                .put("packageName", accessibility.packageName)
                .put("source", "accessibility")
                .put("capturedAt", accessibility.capturedAt?.toString().orEmpty())
                .toString()
        }
        ...
        return JSONObject().put("success", true).put("packageName", packageName).put("source", "usage_stats").put("capturedAt", Instant.ofEpochMilli(latest).toString()).toString()
    }
    ...
    private fun notifications(limit: Int): String {
        ...
        return JSONObject()
            ...
            .put(
                "notifications",
                values.joinToString("\n") { value ->
                    "${value.postedAt} | ${value.packageName} | ${value.title} | ${value.text}"
                },
            )
            .toString()
    }
