# Lulu1 代码索引

- 基准提交：`c68fbb425a28b7c5863d17a70c780eeaa31e3acc`
- 分支：`main`
- 已索引文件：27
- 已索引代码/文本行：4302
- 已发现符号：131

| 文件 | 行数 | 符号数 |
|---|---:|---:|
| `app/build.gradle.kts` | 54 | 0 |
| `app/src/main/AndroidManifest.xml` | 22 | 0 |
| `app/src/main/java/com/jiacimu/lulu/CharacterSettingsScreen.kt` | 181 | 1 |
| `app/src/main/java/com/jiacimu/lulu/ChatHubScreens.kt` | 232 | 3 |
| `app/src/main/java/com/jiacimu/lulu/LuluFinalRootApp.kt` | 219 | 1 |
| `app/src/main/java/com/jiacimu/lulu/LuluRootApp.kt` | 455 | 1 |
| `app/src/main/java/com/jiacimu/lulu/MainActivity.kt` | 19 | 1 |
| `app/src/main/java/com/jiacimu/lulu/MigratedFeatureScreens.kt` | 320 | 5 |
| `app/src/main/java/com/jiacimu/lulu/PublicCharacterWorldBookScreen.kt` | 144 | 1 |
| `app/src/main/java/com/jiacimu/lulu/ai/CompanionModelGateway.kt` | 332 | 10 |
| `app/src/main/java/com/jiacimu/lulu/core/LuluContracts.kt` | 126 | 28 |
| `app/src/main/java/com/jiacimu/lulu/data/InMemoryRepositories.kt` | 192 | 7 |
| `app/src/main/java/com/jiacimu/lulu/data/MigratedDomainStores.kt` | 201 | 21 |
| `app/src/main/java/com/jiacimu/lulu/games/CompleteGamesApp.kt` | 81 | 4 |
| `app/src/main/java/com/jiacimu/lulu/games/LegacyGamesScreen.kt` | 298 | 4 |
| `app/src/main/java/com/jiacimu/lulu/games/LuluGameStore.kt` | 175 | 17 |
| `app/src/main/java/com/jiacimu/lulu/games/LuluGamesApp.kt` | 306 | 1 |
| `app/src/main/java/com/jiacimu/lulu/study/PostgraduateExamApp.kt` | 431 | 1 |
| `app/src/main/java/com/jiacimu/lulu/study/PostgraduateExamStore.kt` | 261 | 25 |
| `app/src/main/java/com/jiacimu/lulu/study/StudyNavigationCompat.kt` | 37 | 0 |
| `app/src/main/res/drawable/ic_lulu.xml` | 12 | 0 |
| `app/src/main/res/values/strings.xml` | 5 | 0 |
| `app/src/main/res/values/styles.xml` | 11 | 0 |
| `scripts/generate_code_index.py` | 33 | 0 |
| `preview/app.js` | 54 | 0 |
| `preview/index.html` | 99 | 0 |
| `preview/styles.css` | 2 | 0 |
